...
function mapBudgetActual(a) { return { id: a.id, roundItemId: a.round_item_id, purchaseDate: fmtDate(a.purchase_date), vendor: a.vendor, quantity: Number(a.quantity), unitPrice: Number(a.unit_price), amount: Number(a.amount), note: a.note, createdBy: a.created_by, createdAt: a.created_at }; }
...
app.post('/api/license/companies/:companyId/bulk-allocate', requireAuth, requireAdmin, async (req, res) => {
    try {
        const { companyId } = req.params;
...
app.post('/api/license/budget-round-items/:itemId/actuals', requireAuth, requireAdmin, async (req, res) => {
    try {
...
app.delete('/api/license/budget-actuals/:id', requireAuth, requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
